...
public class OrderService
{
    ...
    public async Task<Order> CreateOrderAsync(int customerId, List<(int ProductId, int Quantity)> items)
    {
        ...
        foreach (var (productId, quantity) in items)
        {
            ...
            // Check stock via inventory microservice HTTP client
            var hasStock = await _inventoryClient.CheckStockAsync(productId, quantity);
            if (!hasStock)
                throw new InvalidOperationException($"Insufficient stock for {product.Name}");

            // Deduct stock via inventory microservice HTTP client
