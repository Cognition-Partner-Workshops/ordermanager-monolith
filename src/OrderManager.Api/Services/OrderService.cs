...
public class OrderService
{
    ...
    public OrderService(AppDbContext context, InventoryApiClient inventoryApiClient)
    {
        _context = context;
    ...
    public async Task<Order> CreateOrderAsync(int customerId, List<(int ProductId, int Quantity)> items)
    {
        ...
        foreach (var (productId, quantity) in items)
        {
            ...
            // Deduct inventory via the inventory microservice HTTP API
            await _inventoryApiClient.DeductAsync(productId, quantity);
