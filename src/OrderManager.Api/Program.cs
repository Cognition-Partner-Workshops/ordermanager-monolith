...
builder.Services.AddHttpClient<IProductClient, ProductHttpClient>(client =>
{
    var baseUrl = builder.Configuration["ProductService:BaseUrl"] ?? "http://localhost:5300";
    client.BaseAddress = new Uri(baseUrl.EndsWith('/') ? baseUrl : baseUrl + "/");
});
builder.Services.AddHttpClient<ICustomerClient, CustomerHttpClient>(client =>
{
    var baseUrl = builder.Configuration["CustomerService:BaseUrl"] ?? "http://localhost:5400";
    client.BaseAddress = new Uri(baseUrl.EndsWith('/') ? baseUrl : baseUrl + "/");
});
